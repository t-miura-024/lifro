 ...
 * 本 route の動詞は GET/POST に限定する。他動詞の `/api/auth/*` は
 * `src/routes/api.$.tsx` の除外ガードで 404 に落とす（Hono 側に auth 系
 * ルートはなく、到達しても 404 になる同一契約）。
 *
...
async function handleAuth(request: Request): Promise<Response> {
  // 兄弟 `api.$.tsx` と同一の異常系契約: 例外は握り潰さず log＋JSON 500 に正規化する。
