...
// NOTE（E3）: `ProtectedShell` は `status` 由来の表示分岐にしか興味がなく、
// `useTimer()` の全体購読ではタイマー再生中の毎秒更新のたびに Shell 全体が
// 再レンダする。`TimerStatusContext`（プリミティブ値）のみ購読する
// `useTimerStatus()` を使い、status 変化時だけ再レンダする。
// `TimerContext` 本体の分割まではしない小規模対応。
const TimerStatusContext = createContext<TimerStatus | null>(null)
...
export function TimerProvider({ children }: { children: ReactNode }) {
  ...
  // value は useMemo 化する。依存は `state` 全体ではなく必要な断片に分割する
  // （logic-4。小規模・安全な範囲）。毎秒 setState で Provider 自体は再レンダし、
  // `remainingSeconds` 変化時は value 参照も変わるため、`useTimer()` 購読者の
  // 毎秒再レンダ自体を断つには Actions 用 context 分離が必要で、今回は見送る。
  // 救済は `useTimerStatus()`（`TimerStatusContext`）による status のみ購読に留める。
  ...
  return (
    <TimerStatusContext.Provider value={state.status}>
...
/**
 * `status` のみ購読する軽量セレクタ。Shell 等の表示分岐用。
 * 残り時間追従（毎秒更新）では再レンダしない。
 */
